...
function sortArray(arr) {
  ...
  //Escribe tu código aquí
    ...
    for (let i = 0; i < arr.length; i++) {array.push( arr[i].length)}
    
    array.sort()

    arr.filter((e,i)=>{if(e.length<= array[i]) {orden.push(e)}})

    arr.filter((e,i)=>{if(e.length<= array[i] === false) {orden.push(e)}})
